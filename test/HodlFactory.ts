...
import { GullakFactory, MockERC20, GullakERC20 } from "../typechain";
...
describe("Hodl Factory Tests", function () {
  ...
  let factory: GullakFactory;
  let token: MockERC20;
  let bonusToken: MockERC20;
  let accounts: SignerWithAddress[] = [];
  let creator: SignerWithAddress;
  let implementation: GullakERC20;
  ...
  this.beforeAll("Deploy GullakERC20 and mock contract", async () => {
    const GullakERC20 = await ethers.getContractFactory("GullakERC20");
    const contract = await GullakERC20.deploy();
    implementation = contract as GullakERC20;
  ...
  this.beforeAll("Deploy factory", async () => {
    const Factory = await ethers.getContractFactory("GullakFactory");
    factory = (await Factory.deploy(implementation.address)) as GullakFactory;
  });
